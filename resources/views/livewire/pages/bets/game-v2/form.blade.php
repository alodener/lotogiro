...
@if(auth()->user()->type_client == null)
    ...
    <div class="row mb-2 d-none lineCalcBet">
        <div class="col-md-12">
            <input type="text" id="multiplicador" value="" name="multiplicador" hidden>
            <input type="text" id="maxreais" value="" name="maxreais" hidden>
            <input type="text" id="valueId" value="" name="valueId" hidden>
    ...
    <div class="row">
        <div class="col-md-12">
            @if(isset($matriz))
                <h4>Quantidade selecionada:(<span id="quantSelected">0</span>/{{$numbers}})</h4>
                ...
                <div class="col-md-12 automatic-bet">
                    ...
                    @foreach ($busca as $buscas)
                        <button style="margin-top: 1%" wire:click="randomNumbers({{ $buscas['numbers'] }})"
                            class="{{ env('randomNumbersColor') }} btnRandomNumbers" type="button"
                            onclick="limpacampos();" data-multiplicador="{{ $buscas['multiplicador'] }}"
                            data-maxreais="{{ $buscas['maxreais'] }}" data-id="{{ $buscas['id'] }}">
                            {{ $buscas['numbers'] }}
                        </button>
                    @endforeach
                </div>
                        ...
                        @foreach($matriz as $lines)
                            <tr data-toggle="buttons">
                                @foreach($lines as $cols)
                                    <td>
                                        <label id="number_{{$cols}}" class="btn btn-block btn-beat-number btnNumbersGame
                                            {{in_array($cols, $selectedNumbers) ? env('OneNumber2') : 'btn-warning'}}">
                                            <input class="sr-only numberGameChk" type="checkbox" value="{{$cols}}"
                                                   name="numbers[]" id="numberGame" autocomplete="off">
                                            {{$cols}}
                                        </label>
                                    </td>
                                @endforeach
...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            let elBtnNumberGame = $('[data-toggle="buttons"] .btnNumbersGame');

            elBtnNumberGame.on('click', function () {
                // toggle checkbox
                var $chk = $(this).find('[type=checkbox]');
                $chk.prop('checked',!$chk.prop('checked'));

                $('#quantSelected').text($('#numberGame:checked').length);
                ...
                validateCountNumbers();
                return false;
            });

            function validateCountNumbers() {
                let inputs, x, selecionados=0;
                inputs = document.getElementsByClassName('numberGameChk');
                ...
                $('.btnRandomNumbers').map(function (item, value) {
                    values.push([{
                        'id': value.dataset.id ,
                        'value': parseInt($(this).text().trim()),
                        'multiplicador': value.dataset.multiplicador,
                        'maxreais': value.dataset.maxreais
                    }]);
                    return values;
                });

                $.each(values, function(index, item) {
                    if(selecionados === item[0].value){
                        $('.lineCalcBet #multiplicador').val(item[0].multiplicador);
                        $('.lineCalcBet #maxreais').val(item[0].maxreais);
                        $('.lineCalcBet #valueId').val(item[0].id);
                        $('.lineCalcBet').removeClass('d-none');
                        return item[0];
                    }
                });
            }
